...
public class ExcelDataChecker {
     ...
     */
    public static List<HashMap<String, Object>> parseSingleExcelToMap(File excelFile) {
    ...
    private static boolean fileTypeCheck(File excelFile) {
     ...
     * @return List<List < HashMap < String, Object>>> 读取的数据
     */
    public static List<List<HashMap<String, Object>>> parseComplexExcel(File excelFile) {
     ...
     * @return
     */
    private static List<HashMap<String, Object>> singleSheetCheck(Workbook workbook, byte sheetNum) {
     ...
     * @return 写入数据后的工作簿对象
     */
    public static Workbook mapExportData(List<HashMap<String, Object>> dataList, String fileName) {
        // 生成xlsx的Excel
        try {
            ...
            mapKeys = new ArrayList<>(rowMap.keySet());
     ...
     * @return 写入数据后的工作簿对象
     */
    public static <T> Workbook beanExportData(List<T> dataList, String fileName) {
        // 生成xlsx的Excel
        try (Workbook workbook = getWorkbook(fileName)) {
            for (int i = 0; i < dataList.size(); i++) {
                ...
                // 生成Sheet表，写入第一行的列头
                Sheet sheet = buildDataSheet(workbook, fields);
                int rowNum = 1;
                for (int j = 0; j < fields.length; j++) {
                    ...
                    cell.setCellValue(getBeanValue(dataList.get(i), field));
                }
     ...
     * @return 已经写入列头的Sheet
     */
    private static Sheet buildDataSheet(Workbook workbook, Field[] fields) {
     ...
     * @return 已经写入列头的Sheet
     */
    private static Sheet buildDataSheet(Workbook workbook, List<String> keySet) {
        Sheet sheet = workbook.createSheet();
        // 设置列头宽度
     ...
     * @return
     */
    private static void convertDataToRow(HashMap<String, Object> data, Row row) {
        Cell cell;
        int i = 0;
     ...
     */
    public static void exportExcelToDisks(List<HashMap<String, Object>> dataVOList, String exportFilePath, String excelName) {
        // 以文件的形式输出工作簿对象
        try {
            FileOutputStream fileOut = new FileOutputStream(exportFilePath + excelName);
            Workbook workbook = mapExportData(dataVOList, excelName);
            File exportFile = new File(exportFilePath + excelName);
     ...
     * @return list<bean>数据
     */
    public static <T> List<T> readExcelToBean(File file, Class<T> tClass) {
     ...
     * @return list<bean>
     */
    public static <T> List<T> excelToBean(File file, Class<T> tClass) {
     ...
     * @param <T>
     */
    private static <T> void dataTypeCheck(Class<?> type, Field field, T uBean, String dataStr) {
        try {
     ...
     * @throws IOException
     */
    private static Workbook getWorkbook(String fileName) throws IllegalAccessException {
        // 获取Excel后缀名
